...
class _ProviderServicesScreenState extends State<ProviderServicesScreen> {
  ...
  // Lehetséges régi kulcsok – innen próbálunk migrálni
  ...
  Future<void> _load() async {
    ...
    // 2) Ha nincs, próbáljuk a régieket és migráljuk
    if (raw == null || raw.isEmpty) {
      for (final lk in kLegacyKeys) {
        final r = prefs.getString(lk);
        if (r != null && r.isNotEmpty) {
          raw = r;
          await prefs.setString(kNewKey, r); // migrate
          break;
        }
      }
    }

    // 3) Ha még mindig nincs adat, töltsünk be 3 minta tételt (később törölhető)
    if (raw == null || raw.isEmpty) {
      final samples = [
        {
          "id": "S1",
