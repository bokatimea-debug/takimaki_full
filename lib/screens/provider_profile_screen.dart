...
class _S extends State<ProviderProfileScreen> {
  ImageProvider? _photo; int _success = 0;
  @override void initState(){ super.initState(); _load(); }
  Future<void> _load() async {
    final prefs = await SharedPreferences.getInstance();
    final p = await ProfilePhotoLoader.loadAny();
    setState((){ _photo = p; _success = prefs.getInt('provider_success_count') ?? 0; });
  }
  @override
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          FilledButton(onPressed: () async { await Navigator.pushNamed(context, '/provider/edit_profile'); await _load(); }, child: const Text('Profil szerkesztése')),
          ...
          const SizedBox(height: 8),
          OutlinedButton(onPressed: ()=> Navigator.pushNamed(context, '/provider/all_orders'), child: const Text('Összes rendelés')),
        ]),
      ),
    );
  }
