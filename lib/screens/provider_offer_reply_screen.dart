...
class ProviderOfferReplyScreen extends StatefulWidget {
  ...
  @override State<ProviderOfferReplyScreen> createState() => _State();
}
class _State extends State<ProviderOfferReplyScreen> {
  ...
  @override void initState(){ super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_){
      ...
      if (args is Map<String, dynamic>) { data=args; _priceCtrl.text=(data['suggested_price'] ?? '').toString(); setState((){}); }
    });
  }
  ...
  Future<void> _send() async {
    final priceRaw=int.tryParse(_priceCtrl.text.replaceAll(' ','')); if(priceRaw==null || priceRaw<=0){
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Adj meg érvényes árat.'))); return;
    }
    final prefs=await SharedPreferences.getInstance();
    final raw=prefs.getString('provider_requests')??'[]';
    final list=(json.decode(raw) as List).cast<Map<String,dynamic>>();
    final idx=list.indexWhere((e)=> e['id']==data['id']);
    if(idx>=0){ list[idx]['status']='offered'; list[idx]['offered_price']=priceRaw; list[idx]['note']= _noteCtrl.text.trim(); await prefs.setString('provider_requests', json.encode(list)); }
    if(!mounted) return;
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Text(data['address'] ?? ''), Text('${data['date'] ?? ''}  ${data['time'] ?? ''}'),
          const SizedBox(height: 16),
          TextField(controller:_priceCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText:'Ajánlott ár (Ft)'),
          ...
          const SizedBox(height: 8),
          TextField(controller:_noteCtrl, decoration: const InputDecoration(labelText:'Megjegyzés (opcionális)')),
          const Spacer(),
